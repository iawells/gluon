...
class Driver(gluon.backend.Driver):
	...
	# TODO a flask feature is that config must be uppercase, when we move to openstackyness this shouldn't be true any more
	# TODO we should be reading backend-specific config, not plugin-specific config
	# TODO should the backend be supplying most of this config?
        username = config['NEUTRON_USERNAME']
        password = config['NEUTRON_PASSWORD']
        tenant_name = config['NEUTRON_TENANTNAME']
	...
	port_req_body = {'port': {'device_id': '', 'device_owner': '', 'binding:host_id': None}}
	rv = self._client.update_port(port_id, port_req_body)
	pass
	...
	# TODO should we just be transmitting the binding details and adding to it or copying it over
	...
	gluon_port['host'] = neutron_port.get('binding:host_id')
        ...
        # Probably should just go away - don't want Nova bothering about tenancy
        ...
        # in Nova's datastructure.  In some cases it's not really Neutron's problem...
	...
	# TODO think this wants to die - it was intended to be binding_details and clearly it works without
	if 'binding:details' not in gluon_port:
        ...
        vif_active = False
        if (neutron_port['admin_state_up'] is False
            or neutron_port['status'] == 'ACTIVE'):
            vif_active = True
